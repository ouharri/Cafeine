/*jshint -W065 */
var blossomfashionRepeaterRow = function( rowIndex, container, label ){
	'use strict';
...
wp.customize.controlConstructor['blossom-fashion-repeater'] = wp.customize.Control.extend({
	ready: function(){
		'use strict';
	...
	addRow: function( data ){
		...
		if ( template ) {
			...
			newRow = new blossomfashionRepeaterRow(
				control.currentIndex,
    ...
    $(document).on( 'click', 'input[type="font"]', function(){
        ...
        if( ( ! $this.hasClass('ajax-running') ) && $this.siblings( '.font-awesome-list' ).length < 1 ){
            $.ajax({
				...
				url : ajaxurl,
				data: {
					action             : 'blossom_fashion_get_fontawesome_ajax',
					blossom_fashion_customize_nonce: blossom_fashion_customize.nonce
				},
				beforeSend: function(){
