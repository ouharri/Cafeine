 ...
 * @subpackage Skt_Templates/app/helpers
 */

class Skt_Templates_Render_Helper {
	/**
	...
	public function get_view( $name = '', $args = array() ) {
		ob_start();
		$file = SKB_PATH . '/core/app/views/' . $name . '-page.php';
		if ( ! empty( $args ) ) {
	...
	public function render_option( $option = array() ) {
		...
		switch ( $option['type'] ) {
			...
			case 'title':
				...
				break;
			case 'custom':
				return apply_filters( 'sktb_custom_control_' . $option['id'], '' );
				break;
			case 'link':
			...
			case 'password':
				...
				break;
			default:
				return __( 'No option found for provided type', 'skt-templates' );
				break;
		}
