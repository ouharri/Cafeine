<?php
/**
 * The abstract class for SKT Templates Modules.
 *
 * @link       https://www.sktthemes.org
 * @since      1.0.0
 *
 * @package    Skt_Templates
 * @subpackage Skt_Templates/app/abstract
 */

abstract class Skt_Templates_Module_Abstract {
	...
	/**
	 * Has an instance of the Skt_Templates_Loader class used for adding actions and filters.
	 *
	 * @since   1.0.0
	 * @access  protected
	 * @var     Skt_Templates_Loader $loader A instance of Skt_Templates_Loader.
	 */
	protected $loader;

	/**
	 * Has an instance of the Skt_Templates_Model class used for interacting with DB data.
	 *
	 * @since   1.0.0
	 * @access  protected
	 * @var     Skt_Templates_Model $model A instance of Skt_Templates_Model.
	 */
	protected $model;
	 ...
	 * @since   1.0.0
	 * @access  protected
	 * @var     string $version The current version of SKT Templates.
	 */
	protected $version;
	...
	/**
	 * Skt_Templates_Module_Abstract constructor.
	 *
	 * @since   1.0.0
	 * @access  public
	 */
	public function __construct() {
		$this->slug = str_replace( '_', '-', strtolower( str_replace( '_SKTB_Module', '', get_class( $this ) ) ) );
	}
	 ...
	 * @access  public
	 *
	 * @param Skt_Templates_Loader $loader The loader class used to register action hooks and filters.
	 */
	public function register_loader( Skt_Templates_Loader $loader ) {
		$this->loader = $loader;
	 ...
	 * @access  public
	 *
	 * @param Skt_Templates_Model $model The loader class used to register action hooks and filters.
	 */
	public function register_model( Skt_Templates_Model $model ) {
		$this->model = $model;
	}
	...
	final public function set_enqueue( $version ) {
		...
		$this->loader->add_action( 'sktb_admin_enqueue_styles', $this, 'set_admin_styles' );
		$this->loader->add_action( 'sktb_admin_enqueue_scripts', $this, 'set_admin_scripts' );

		$this->loader->add_action( 'sktb_public_enqueue_styles', $this, 'set_public_styles' );
		$this->loader->add_action( 'sktb_public_enqueue_scripts', $this, 'set_public_scripts' );
	}
		...
		if ( ! empty( $enqueue ) ) {
			if ( isset( $enqueue['css'] ) && ! empty( $enqueue['css'] ) ) {
				...
				foreach ( $enqueue['css'] as $file_name => $dependencies ) {
					...
					$id                = 'sktb-module-' . $prefix . '-css-' . str_replace( ' ', '-', strtolower( $this->name ) ) . '-' . $order;
					$map[ $file_name ] = $id;
					wp_enqueue_style(
						$id,
						$resource,
		...
		if ( ! empty( $enqueue ) ) {
			if ( isset( $enqueue['js'] ) && ! empty( $enqueue['js'] ) ) {
				...
				foreach ( $enqueue['js'] as $file_name => $dependencies ) {
					...
					$id                = 'sktb-module-' . $prefix . '-js-' . $module_dir . '-' . $order;
					$map[ $file_name ] = $id;
	...
	protected function is_lucky_user( $percent = 10 ) {
		...
		if ( $force_beta ) {
			update_option( 'sktb_beta_show_' . $this->get_slug(), 'yes' );

			return true;
		}
		$luck = get_option( 'sktb_beta_show_' . $this->get_slug() );
		if ( ! empty( $luck ) ) {
			return $luck === 'yes';
		}
		$luck = rand( 1, 100 );
		...
		$luck = $luck <= $percent;
		update_option( 'sktb_beta_show_' . $this->get_slug(), $luck ? 'yes' : 'no' );
